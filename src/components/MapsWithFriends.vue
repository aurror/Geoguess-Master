<template>
  <div>
    <div id="map" @mouseover="mouseOverMap" @mouseleave="mouseOutMap"></div>
    <v-btn
      id="hide-map-button"
      v-if="
        viewport.width < 450 &&
    ...
    <button
      id="guess-button"
      :disabled="
        state.selectedLatLng === null || state.isGuessButtonClicked || !isReady
      "
      v-if="
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    const state = reactive<{
      ...
      polylines: [],
      summary: [],
      strokeColors: ["#F44336", "#76FF03", "#FFEB3B", "#FF4081", "#18FFFF"],
      map: null,
      room: null,
      selectedLatLng: null,
      distance: 0,
    ...
    function calculateDistance(): void {
      ...
      state
        .room!.child("round" + props.round + "/player" + props.playerNumber)
        .set(state.distance);
    ...
    function setInfoWindow(playerName: string, distance: number): void {
      let infoWindow = new google.maps.InfoWindow({
        content:
          "<b>" + playerName + "</b>" + " is <b>" + distance + "</b> km away!",
      });
    ...
    function mouseOutMap(): void {
      if (!state.isSelected && viewport.width > 450) {
        ...
        document.getElementById("map")!.style.transform = "scale(0.4762)";
      }
    }
        ...
        if (snapshot.hasChild("active")) {
          ...
          if (
            ...
            snapshot.child("guess").forEach((childSnapshot) => {
              ...
              let playerName = snapshot
                .child("playerName")
                .child(childSnapshot!.key!)
                .val();
              let distance = snapshot
                .child("round" + props.round + "/player" + j)
                .val();
